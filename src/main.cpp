/**
 * @file main.cpp
 * @author Ivo de Maijer
 * @brief Controlling a 1DOF helicopter angle using a PID controller using a FreeRTOS implementation
 * @version 1
 * @date 2022-04-28
 * 
...
// Settings
static const int START_ANGLE = -30.0;  /// The offset of the 1DOF arm from start to hovering at 0 degrees
 ...
 * For use with InduSoft SCADA, the serialRead function reads data sent
 * from InduSoft to the Arduino. The data is read in the format 
 * {'off/on','manual/auto', 'set_point', 'kp','ki','kd'} 
 * and parses this data to the corresponding variables
 * This function is executed around 10 times per secondS (~10Hz)S
 * @param parameter 
 */
 ...
 * Writes data from the arduino to the computer for use in
 * InduSoft SCADA system. The data has the format of:
 * 'angle,pwm,sp;' and parsed in InduSoft to the corresponding tags (variables)
 * The function runs around 5 times per second (~5Hz)
 * @param parameter 
 */
...
void serialWrite(void *parameter)
{
  while (1)
  {
    ...
    if (xSemaphoreTake(xSerialSemaphore, (TickType_t)5) == pdTRUE)
    {
      ...
      // Data will be send in the following message format:
      // 'angle,pwm,sp;' // with ';' being the end marker (end of message)
      // Concatentate string using sprintf and write the string to serial port
      sprintf(str, "%s,%d,%d;\n", buf_angle, pwm, set_point);
      Serial.print(str);
      xSemaphoreGive(xSerialSemaphore); // Now free or "Give" the Serial Port for others.
    }
    vTaskDelay(freqToTime(5) / portTICK_PERIOD_MS); // Do ~5 times per second (~5Hz)
  }
}
/**
...
#ifdef TESTING
  // Timing test, calculates the frequency all tasks are executed.
...
/**
 * @brief Updates encoder value on interrupt
 * 
 * Is attached to INT0 and INT1 (pin 2 and pin 3)
 * and executed on CHANGE of either interrupts
 * thus, 1600 interrupts per revolution
 */
void updateEncoder()
{
  ...
  // BITWISE shift left two bits and perform OR to add encoded to lastEncoded value
...
void emergencyStop()
{
  analogWrite(MOTOR, 0); // Turn fan off
  TIMSK1 = 0x00;         // Stop timer interrupt
...
/**
 * @brief Takes the serial data and splits the data in to its parts
 */
void parseData()
{
  ...
  if (running == 0)
  {
    ...
    vTaskSuspend(xPIDHandle); // Suspend PID task
  ...
  {
    digitalWrite(LED_BUILTIN, HIGH);
    vTaskResume(xPIDHandle); // Resume PID task
    TIMSK1 = 0x02;                                // Enable Timer1 compare interrupt TIMSK1 = 0000 0010
    attachInterrupt(INT0, updateEncoder, CHANGE); // Re-attach both encoder intterupts
    attachInterrupt(INT1, updateEncoder, CHANGE);
  }

  if (auto_mode == 0) // If the system is in manual mode
  {                                
